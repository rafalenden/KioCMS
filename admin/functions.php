...
function save_config($holder)
{
	global $blocks, $cfg, $sql, $form, $lang_system, $lang_admin;

	if ($form['blocks'] !== false)
		$form['blocks'] = implode(', ', $_POST['blocks'] ? $form['blocks'] : $blocks);
...
function form_blocks()
{
	...
	$query = sql_query('SELECT id, name, codename, side FROM '.DB_PREFIX.'blocks WHERE type != 0 ORDER BY position');
	while ($row = sql_fetch_assoc($query))
	{
		if ($row['side'] == 'L') $left_blocks[] = $row;
		else $right_blocks[] = $row;
	}
