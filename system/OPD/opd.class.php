...
function opdErrorHandler(PDOException $exc)
{
	echo '<br/><b>Open Power Driver internal error #'.$exc->getCode().': </b> '.$exc->getMessage().'<br/>
		Query used: <i>'.OPD::$lastQuery.'</i><br/>';
}
...
class OPD
{
	...
	public function exec($statement, $cache_id = NULL)
	{
		if(!is_null($cache_id))
		{
			$this->clearCache($cache_id);
//			$stmt = $this->prepare($statement);
//			$stmt->bindValue(':id', $cache_id, PDO::PARAM_INT);
//			return $stmt->execute();
		}
	...
	public function setCache($id, $peroid = null, $prepare = true)
	{
		$this->cache = true;
		$this->cacheId = $id;
	...
	public function putCacheContent($id, &$content = null)
	{
		if (!$content) return;

		return file_put_contents($this->cacheDir.$id.'.php', OPD_CACHE_PREFIX.serialize($content));
		//return fwrite(fopen($this->cacheDir.$id.'.php', 'w'), serialize($content));
	} // end putCacheContent();

	public function getCache($id, $peroid = NULL)
	{
		//return unserialize(file_get_contents($this->cacheDir.$id.'.php', false, null, strlen(OPD_CACHE_PREFIX)));
		return ($content = @file_get_contents($this->cacheDir.$id.'.php', false, null, strlen(OPD_CACHE_PREFIX))) ? unserialize($content) : false;
	} // end getCache();
