...
class OPDStatement implements iopdStatement, Iterator
{
	...
	public function __construct(OPD $opd, PDOStatement $stmt, $query = NULL)
	{
		$this->stmt = $stmt;
		$this->opd = $opd;
		$this->query = $query;
	...
	public function fetch($fetchStyle = PDO::FETCH_BOTH, $orientation = PDO::FETCH_ORI_NEXT, $offset = NULL)
	{
		...
		$this->closeCursor(); // FIXME: could be done better

	} // end fetch();

	// TODO: Add $this->closeCursor()
	public function fetchAll($fetchStyle = PDO::FETCH_BOTH, $columnIndex = 0)
	{
...
class opdCachedStatement implements iopdStatement, Iterator
{
	...
	public function __construct(OPD $opd, $cacheStatus, $param2 = NULL, $param3 = NULL)
	{
		$this->opd = $opd;
		...
		if($this->cache)
		{
			...
			if($this->cacheId != NULL)
			{
				...
				$this->data = unserialize(file_get_contents($this->cacheDir.$this->cacheId.'.php', false, null, strlen(OPD_CACHE_PREFIX)));
				$this->opd->endTimer();
			}
		}
		else
	...
	public function closeCursor()
	{
		...
		if(!$this->cache)
		{
			file_put_contents($this->cacheDir.$this->cacheId.'.php', OPD_CACHE_PREFIX.serialize($this->data));
			return $this->stmt->closeCursor();
		}
		return 1;
	} // end closeCursor();
...
class opdPreparedCacheStatement extends opdCachedStatement
{
	...
	public function __construct(OPD $opd, Array $itemList, $stmt, $query)
	{
		$this->query = $query;
		$this->opd = $opd;
		$this->cacheDir = $this->opd->getCacheDirectory();
